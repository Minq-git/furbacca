...
- **`eye-track`** — (alias from **setup-fresh.sh** on the Pi) runs **scripts/eye-track.sh**. Start/stop AI camera tracking (eyes follow you): **`eye-track on`** | **`eye-track off`** | **`eye-track status`**. Run in foreground with args: **`eye-track run --print-every 30`**. **From Mac:** add an alias so you can pass host + action: `alias eye-track='~/Documents/Code/Furbacca/scripts/eye-track.sh'` then run **`eye-track furbacca.local on`** (or `off`, `status`). Use **furbacca.local** (mDNS); if you use **furbacca.lan**, add the Pi’s IP to `/etc/hosts`. If SSH says "Host key verification failed", run `ssh -o StrictHostKeyChecking=accept-new minqz@furbacca.local` once to accept the key. Uses **FURBACCA_SSH_USER** (default `minqz`) for SSH.
...
- **Network dead after brownout (can't SSH, furbacca.local doesn't resolve):** Brownouts can corrupt `wpa_supplicant.conf` or leave the Wi‑Fi stack hung. **If the Pi is running and eyes are on:** hold **head + belly for 30 seconds** — the nervous system runs **`./scripts/heal-network.sh`** (restarts NetworkManager/dhcpcd, cycles wlan0, and restores Wi‑Fi from a backup if present). **One-time prep** (when you have SSH): copy a known-good Wi‑Fi config to the boot partition so it survives brownouts: **`sudo cp /etc/wpa_supplicant/wpa_supplicant.conf /boot/wpa_supplicant.conf`**. Log: `~/network_heal.log`. If the Pi is fully offline (no IP), use a monitor + keyboard or re-flash the SD.
