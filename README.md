...
- **Audio works after reboot but stops after wake-furbacca until next reboot:** Something in the stack can put the I2S device in a bad state. Try **skipping the amixer volume step:** **`FURBACCA_SKIP_AMIXER=1 wake-furbacca`**. If that doesn’t fix it, try **disabling eye tracking** to see if the camera process is the cause: **`wake-furbacca --no-eye-track`** (or **`FURBACCA_EYE_TRACK=0 wake-furbacca`**). If audio works with **--no-eye-track**, the eye-tracking (camera) process is likely affecting I2S; if it still fails, the cause is elsewhere (Node/eyes/Matter). Check whether PulseAudio or pipewire is running. Rebooting always restores I2S.
