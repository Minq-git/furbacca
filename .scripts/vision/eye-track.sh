#!/usr/bin/env bash
# Start, stop, or run the eye-tracking process (vision/py/camera/camera_track.py) on the Pi.
...
NS_EVENTS_PORT="${VISION_UDP_PORT:-5006}"
  ...
  on)
    ...
    if pgrep -f "$TRACK_PATTERN" >/dev/null 2>&1; then
      ...
      if kill -0 "$PID" 2>/dev/null; then
        ...
        python3 -c "import socket; s=socket.socket(socket.AF_INET,socket.SOCK_DGRAM); s.sendto(b'{\"event\":\"eye_tracking_started\"}', ('127.0.0.1', $NS_EVENTS_PORT)); s.close()" 2>/dev/null || true
      else
        ...
        python3 -c "import socket; s=socket.socket(socket.AF_INET,socket.SOCK_DGRAM); s.sendto(b'{\"event\":\"eye_tracking_stopped\"}', ('127.0.0.1', $NS_EVENTS_PORT)); s.close()" 2>/dev/null || true
        exit 1
      fi
    fi
    ;;
