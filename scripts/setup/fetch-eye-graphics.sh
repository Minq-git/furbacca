#!/usr/bin/env bash
# Fetch eye assets (iris, sclera, etc.) from Adafruit Pi_Eyes into vision/py/assets/graphics/.
# Removes unused files (SVGs, lid.png, uv.png) so only assets used by vision/py/main_eyes.py remain.
# Run from repo root. Does not overwrite existing files.

set -e
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
GRAPHICS_DIR="${REPO_DIR}/vision/py/assets/graphics"
...
# Remove assets not used by vision/py/main_eyes.py
...
echo "Done. Contents of vision/py/assets/graphics:"
ls -la "$GRAPHICS_DIR"
echo ""
echo "vision/py/main_eyes.py will use iris.jpg (or eye.png if present) on both displays."
