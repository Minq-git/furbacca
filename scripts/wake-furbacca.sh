...
# Eye-tracking (camera_track.py) starts by default on the Pi; use --no-eye-track or FURBACCA_EYE_TRACK=0 to disable.
...
cleanup() {
  ...
  if [[ "$EYE_TRACK_STARTED_BY_US" -eq 1 ]]; then
    "$REPO_DIR/scripts/eye-track.sh" off 2>/dev/null || true
  fi
