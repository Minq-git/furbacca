...
async function main(): Promise<void> {
  ...
  if (!fanControl.isInitialized()) {
    ...
    console.error("  - GPIO access: sudo usermod -aG gpio $USER (then log out/in) or run with sudo");
    console.error("  - BCM 24 (physical pin 18) wiring: 1kΩ → 2N2222 base, fan 5V→collector, emitter GND");
    process.exit(1);
  }
  console.log("Fan init OK. Soft-start 0→100% over 2s, then 100% for", RUN_SEC, "s...");
