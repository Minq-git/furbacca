...
# On the Pi:  ./scripts/monitor-zram.sh
# From Mac:   ./scripts/monitor-zram.sh furbacca.local   (SSH to Pi and run there)
...
if [[ -n "$1" && ( "$1" == *.* || "$1" == "furbacca" ) ]]; then
  ...
  exec ssh -t "$SSH_USER@$REMOTE_HOST" 'cd ~/furbacca && ./scripts/monitor-zram.sh'
fi

cd "$REPO_DIR"

echo "--- Furbacca zRAM Monitor (Press Ctrl+C to stop) ---"
echo "Format: [Raw Data] -> [Compressed] (Ratio) | SD Swap Used"
echo ""
...
while true; do
  ...
  printf "\r\033[K"
  printf "zRAM: %s KB -> %s KB | Ratio: %s:1 | SD swap used: %s KB" \
         "$RAW_KB" "$COMPR_KB" "$RATIO" "${SD_SWAP:-0}"
