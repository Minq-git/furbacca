...
let matterLobe: {
	notifyTouch(sensor: "head" | "belly" | "shiver", active: boolean): void;
	close(): Promise<void>;
} | null = null;
...
function chipToolOn(): void {
	...
	const child = spawn(
		"chip-tool",
		["onoff", "on", chipToolNodeId, chipToolEndpoint],
		{
			stdio: "ignore",
			detached: true,
		},
...
if (chipToolNodeId)
	console.log(
		substitute(msg.nervous_system.chip_tool_belly, {
			nodeId: chipToolNodeId,
			endpoint: chipToolEndpoint,
		}),
	);
if (!headHw.ok && headHw.message)
	console.log(
		substitute(msg.nervous_system.head_touch_error, {
			message: headHw.message,
		}),
	);
if (!bellyHw.ok && bellyHw.message)
	console.log(
		substitute(msg.nervous_system.belly_touch_error, {
			message: bellyHw.message,
		}),
	);
if (!vibeHw.ok && vibeHw.message)
	console.log(
		substitute(msg.nervous_system.vibration_error, { message: vibeHw.message }),
	);
...
function startWarmupThenOpen(
	matterResultPromise: Promise<MatterStartResult | undefined>,
): void {
	...
	process.stdout.write(
		"\r" +
			CLEAR_LINE +
			warmupBar(WARMUP_STEPS, WARMUP_STEPS, "Opening eyes.") +
			"\n",
	);
	warmupComplete = true;
...
const matterLobeStatusLines = msg.matter_lobe.status_order.map((key) => {
	if (key === "checking_port")
		return substitute(matterLobeStatus[key], { port: "5540" });
	if (key === "online" && hasCachedPairing)
		return matterLobeStatus.online_short ?? "Online.";
