 ...
 * Output: voice/assets/test_tone_1w8ohm.wav
 * Run from repo root: npx ts-node voice/ts/generate-test-tone.ts  (or node dist/voice/ts/generate-test-tone.js after build)
 *
...
function writeWavHeader(
	...
	offset += 4;
	header.writeUInt32LE(36 + dataLength, offset);
	offset += 4;
...
function main(): void {
	...
	const assetsDir = path.join(repoRoot, "voice", "assets");
	...
	console.log(
		`Frequencies: ${FREQUENCIES_HZ.join(", ")} Hz; ${TONE_DURATION_S}s each, ${SILENCE_DURATION_S}s silence.`,
	);
}
