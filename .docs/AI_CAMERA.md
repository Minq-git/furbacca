...
2. **Streaming results**  
   - **Option A — UDP to eyes:** Camera process sends UDP messages to the same host:port the eyes use (e.g. `localhost:5005`), with the same `look` payload format (e.g. `look x y` or whatever **vision/py/main_eyes.py** expects). Then eyes track without the nervous system.  
...
- Furbacca eyes UDP: **vision/ts/eye_bridge.ts**, **vision/py/main_eyes.py** (e.g. `look` with `x`, `y` in -1..1).  
