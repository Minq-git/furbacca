...
export function createFurbaccaExtendedColorLight(
  ExtendedColorLightDeviceDefinition: { with: (...behaviors: unknown[]) => unknown },
  ExtendedColorLightRequirements: { IdentifyServer: new (...args: unknown[]) => { endpoint: unknown } }
): unknown {
  ...
  class FurbaccaIdentifyServer extends BaseIdentifyWithTrigger {
    triggerEffect(request: Identify.TriggerEffectRequest): void {
      const endpoint = (this as unknown as { endpoint: { env: { maybeGet: (k: unknown) => unknown } } }).endpoint;
      const eyes = endpoint.env.maybeGet(EyeBridgeForIdentify) as EyeBridge | undefined;
      if (!eyes) return;
