...
const CORRUPT_GENERAL_DIAGNOSTICS_KEYS = [
	"__features__",
			...
			for (const file of files) {
				try {
					...
					const isCorrupt =
						...
						CORRUPT_GENERAL_DIAGNOSTICS_KEYS.some((k) =>
							decoded.endsWith(`.${k}`),
						);
...
export class MatterLobe {
	...
	async close(): Promise<void> {
		...
		if (
			this.matterNode &&
			typeof (this.matterNode as { close?: () => Promise<void> }).close ===
				"function"
		) {
	...
	async start(options?: {
		...
		const doStart = async (): Promise<void> => {
			...
			const defaultDef = Logger.logger.find(
				(l) => l.logIdentifier === "default",
			);
			if (defaultDef) {
				...
				defaultDef.log = (level: number, formattedLog: string) => {
					const shouldEmit =
						level >= LogLevel.WARN || pairingQrPattern.test(formattedLog);
			...
			const { GenericSwitchDeviceDefinition, GenericSwitchRequirements } =
				devices;
			const {
			...
			const { createFurbaccaExtendedColorLight, EyeBridgeForIdentify } =
				await import("./furbacca_identify_server.js");
			const FurbaccaExtendedColorLightDeviceDefinition =
				createFurbaccaExtendedColorLight(
					...
					ExtendedColorLightRequirements as {
			...
			status(
				substitute(msg.matter_lobe.status.checking_port, {
					port: String(MATTER_UDP_PORT),
				}),
			);
			if (!(await isUdpPortFree(MATTER_UDP_PORT))) {
				console.warn(
					substitute(msg.matter_lobe.udp_port_in_use, {
						port: String(MATTER_UDP_PORT),
					}),
				);
				throw new Error(
					`Matter requires UDP port ${MATTER_UDP_PORT}; it is already bound.`,
				);
			}
			...
			const node = this.matterNode as Awaited<
				ReturnType<typeof ServerNode.create>
			>;
			...
			if (ev.onOff?.onOff$Changed?.on) {
				ev.onOff.onOff$Changed.on((v) => {
					...
					console.log(
						substitute(msg.matter_lobe.onoff_changed, { value: String(isOn) }),
					);
					if (isOn) {
						this.eyes.impulse();
			...
			if (ev.levelControl?.currentLevel$Changed?.on) {
				ev.levelControl.currentLevel$Changed.on((v) => {
					...
					console.log(
						substitute(msg.matter_lobe.brightness_set, {
							level: String(level),
						}),
					);
			...
			if (ev.colorControl?.currentHue$Changed?.on)
				ev.colorControl.currentHue$Changed.on((v) => {
					...
					if (hue == null) return;
					console.log(
						substitute(msg.matter_lobe.hue_set, { hue: String(hue) }),
					);
					applyHueToSpecies(hue, this.eyes);
				});
			// Color temperature (e.g. "Warm White" / "Cool White"): high mireds = warm → human, low = cool → demon
			const evColor = ev.colorControl as
				| Record<
						string,
						{ on?: (cb: (v: unknown) => void) => void } | undefined
				  >
				| undefined;
			...
			const identify = (
				ev as Record<
					string,
					{
						startIdentifying?: { on: (cb: () => void) => void };
						identifyTime$Changed?: { on: (cb: (v: unknown) => void) => void };
					}
				>
			).identify;
			...
			const SwitchDevice = GenericSwitchDeviceDefinition.with(
				GenericSwitchRequirements.SwitchServer,
			);
			const setMomentaryFeatureMap = async (
				...
				try {
					...
					console.warn(
						substitute(msg.matter_lobe.switch_featuremap_warn, {
							error: String(e),
						}),
					);
				}
			...
			const attachIdentifyStub = (
				...
				const events = endpoint.events as Record<
					string,
					{
						startIdentifying?: { on: (cb: () => void) => void };
						identifyTime$Changed?: { on: (cb: (v: unknown) => void) => void };
				...
				if (id?.startIdentifying?.on)
					id.startIdentifying.on(() =>
						console.log(substitute(msg.matter_lobe.identify_on, { label })),
					);
				else if (id?.identifyTime$Changed?.on)
					id.identifyTime$Changed.on((v) => {
						if ((v as number) > 0)
							console.log(substitute(msg.matter_lobe.identify_on, { label }));
					});
			};

			const bellyEndpoint = await node.add(
				SwitchDevice as unknown as Parameters<typeof node.add>[0],
				{
					id: "belly",
				} as Parameters<typeof node.add>[1],
			);
			...
			const headEndpoint = await node.add(
				SwitchDevice as unknown as Parameters<typeof node.add>[0],
				{
					id: "head",
				} as Parameters<typeof node.add>[1],
			...
			const shakeEndpoint = await node.add(
				SwitchDevice as unknown as Parameters<typeof node.add>[0],
				{
					id: "shake",
				} as Parameters<typeof node.add>[1],
			...
			status(
				options?.pairingAlreadyShown
					? ((msg.matter_lobe.status as { online_short?: string })
							.online_short ?? "Online.")
